            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                if (startTestBtn) {
                    startTestBtn.addEventListener('click', async function() {
                        ...
                        if (!inputValue) {
                            alert('${isFarsi ? 'لطفا IP یا دامنه وارد کنید' : '请输入IP或域名'}');
                            return;
                        }
                        ...
                        for (const target of targets) {
                            ...
                            testStatus.textContent = '${isFarsi ? 'در حال تست' : '测试中'}: ' + (completed + 1) + '/' + total + ' - ' + target;
                            ...
                            if (result.success && result.colo) {
                                result.nodeName = nodeName || ('CF-' + result.colo);
                            } else {
                                result.nodeName = nodeName || host;
                            }
                            ...
                            checkbox.dataset.index = testResults.length - 1;
                            ...
                            if (result.success) {
                                ...
                                info.innerHTML = '<span style="color: #00ff00;">' + host + ':' + port + '</span>' + coloDisplay + ' <span style="color: #ffff00;">' + result.latency + 'ms</span>';
                            } else {
                                ...
                                info.innerHTML = '<span style="color: #ff4444;">' + host + ':' + port + '</span> <span style="color: #ff6666; font-size: 12px;">' + errorDetail + '</span>' + testUrlDisplay;
                            }
                        ...
                        if (selectedItems.length > 0) {
                            ...
                            try {
                                const configData = {
                                    ...
                                    preferredIPs: newValue,
                                    preferredIPsURL: document.getElementById('preferredIPsURL').value,
                ...
                const coloMap = {
                    ...
                    'HKG': '香港', 'TPE': '台北', 'NRT': '东京成田', 'HND': '东京羽田', 'KIX': '大阪',
                    'ICN': '首尔', 'SIN': '新加坡', 'BKK': '曼谷', 'KUL': '吉隆坡', 'CGK': '雅加达',
                    'MNL': '马尼拉', 'HAN': '河内', 'SGN': '胡志明', 'BOM': '孟买', 'DEL': '新德里',
                    'MAA': '金奈', 'BLR': '班加罗尔', 'CCU': '加尔各答', 'HYD': '海得拉巴',
                    'LHR': '伦敦', 'CDG': '巴黎', 'FRA': '法兰克福', 'AMS': '阿姆斯特丹', 'MUC': '慕尼黑',
                    'ZRH': '苏黎世', 'VIE': '维也纳', 'MXP': '米兰', 'FCO': '罗马', 'MAD': '马德里',
                    'BCN': '巴塞罗那', 'LIS': '里斯本', 'DUB': '都柏林', 'BRU': '布鲁塞尔', 'CPH': '哥本哈根',
                    'ARN': '斯德哥尔摩', 'OSL': '奥斯陆', 'HEL': '赫尔辛基', 'WAW': '华沙', 'PRG': '布拉格',
                    'SYD': '悉尼', 'MEL': '墨尔本', 'BNE': '布里斯班', 'PER': '珀斯', 'AKL': '奥克兰',
                    'GRU': '圣保罗', 'GIG': '里约', 'EZE': '布宜诺斯艾利斯', 'SCL': '圣地亚哥', 'BOG': '波哥大',
                    'MEX': '墨西哥城', 'YYZ': '多伦多', 'YVR': '温哥华', 'YUL': '蒙特利尔',
                    'JNB': '约翰内斯堡', 'CPT': '开普敦', 'CAI': '开罗', 'DXB': '迪拜', 'DOH': '多哈',
                    'TLV': '特拉维夫', 'IST': '伊斯坦布尔', 'DME': '莫斯科', 'LED': '圣彼得堡',
                    'CTU': '成都', 'CKG': '重庆', 'WUH': '武汉', 'CAN': '广州', 'SZX': '深圳',
                    'SHA': '上海虹桥', 'PVG': '上海浦东', 'PEK': '北京首都', 'PKX': '北京大兴',
                    'TSN': '天津', 'SHE': '沈阳', 'DLC': '大连', 'TAO': '青岛', 'NKG': '南京',
                    'HGH': '杭州', 'XMN': '厦门', 'FOC': '福州', 'CSX': '长沙', 'CGO': '郑州',
                    'XIY': '西安', 'KMG': '昆明', 'NNG': '南宁', 'HAK': '海口', 'SYX': '三亚'
                };
